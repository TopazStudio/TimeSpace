...
if (is_dir($dir)){
    if ($dh = opendir($dir)){
        while (($file = readdir($dh)) !== false){
            ...
            $types[$type] = 'App\\Http\\GraphQL\\Types\\' . $file[0];
//            GraphQL::schema()->query($type, 'Queries\\' . $file[0]);
        }
        closedir($dh);
    }
}
...
if (is_dir($dir)){
    if ($dh = opendir($dir)){
        while (($file = readdir($dh)) !== false){
            ...
            $queries[$type] = 'App\\Http\\GraphQL\\Queries\\' . $file[0];
//            GraphQL::schema()->query($type, 'Queries\\' . $file[0]);
        }
        closedir($dh);
    }
...
if (is_dir($dir)){
    if ($dh = opendir($dir)){
        while (($file = readdir($dh)) !== false){
            ...
            $mutations[$type] = 'App\\Http\\GraphQL\\Mutations\\' . $file[0];
        }
        closedir($dh);
    }
}
